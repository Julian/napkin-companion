...
- **$\mathbb{N}$ contains $0$.** Upstream's "Conventions and notations"
  ...
  claim about $\mathbb{N}$ — unlike the convention above, they assert nothing
  Mathlib denies, and rewriting them would be churn. They are addressed
  instead by naming the offset once, in Metric spaces where sequences and
  `Filter.atTop` first meet: `atTop` is generated by the sets $\{n \mid n \ge
  N\}$, so it cannot see a finite prefix, and convergence is a property of the
  tail. (Most $x_1, \dots, x_n$ in the book are finite tuples — coordinates,
