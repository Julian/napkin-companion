...
We call a function $`\operatorname{Norm} \colon R \to \mathbb{Z}_{\geq 0}` that satisfies the two conditions above an *Euclidean norm*, and an integral domain $`R` that has a norm a *Euclidean domain*.
