...
The chapter is careful to keep *irreducible* ("does not factor") apart from *prime* ("divides a product only by dividing a factor"), because in a general domain they differ; what a UFD buys is that they coincide, which is `UniqueFactorizationMonoid.irreducible_iff_prime`.
Put that to work and recover Euclid's lemma in any UFD: an irreducible dividing a product divides one of the factors.

:::exercise
```lean
example (R : Type*) [CommRing R] [IsDomain R] [UniqueFactorizationMonoid R]
    (p a b : R) (hp : Irreducible p) (h : p ∣ a * b) : p ∣ a ∨ p ∣ b := by
  sorry
```
:::

:::hint
Convert the hypothesis into `Prime p` with the equivalence, and then note how `Prime` is packaged — it is a triple, whose third component is exactly the divides-a-factor statement, so `.2.2` projects it out.
:::

:::solution
```lean
example (R : Type*) [CommRing R] [IsDomain R] [UniqueFactorizationMonoid R]
    (p a b : R) (hp : Irreducible p) (h : p ∣ a * b) : p ∣ a ∨ p ∣ b :=
  -- Irreducible ⟹ prime in a UFD; primality is the third field of `Prime`.
  (UniqueFactorizationMonoid.irreducible_iff_prime.mp hp).2.2 a b h
```
:::
...
The reason a division algorithm forces principality is Bézout: running Euclid's algorithm on $`a` and $`b` writes their gcd as a combination $`ax + by`, so the ideal $`(a, b)` collapses onto the single generator $`\gcd(a,b)`.
Mathlib runs the algorithm for you and records the output as `EuclideanDomain.gcd_eq_gcd_ab`, which produces the two coefficients.
Prove the collapse, in the form: the gcd lies in the ideal generated by $`a` and $`b`.

:::exercise (chili := 1)
```lean
example (R : Type*) [EuclideanDomain R] [DecidableEq R] (a b : R) :
    EuclideanDomain.gcd a b ∈ Ideal.span ({a, b} : Set R) := by
  sorry
```
:::

:::hint
Rewrite with Bézout, and the goal becomes membership of a sum of two multiples — `Ideal.add_mem` splits it, `Ideal.mul_mem_right` strips each coefficient, and `Ideal.subset_span` places $`a` and $`b` in their own span.
:::
