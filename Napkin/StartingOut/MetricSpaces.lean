...
One mismatch to notice and then stop worrying about: the prose above writes a sequence as $`x_1, x_2, \dots`, while `x : ℕ → M` starts at `x 0`.
Nothing depends on which, and `Filter.atTop` is exactly why — it is generated by the sets $`\{n \mid n \geq N\}`, so it cannot see any finite prefix of the sequence.
Convergence is a property of the tail, and dropping or adding a first term changes neither the statement nor its proof.
That is worth having in mind generally: when a formalization renumbers something, the question to ask is whether any statement can tell.
