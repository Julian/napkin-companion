...
*Two distinctions the prose does not make.*
"Finite set" is one word in mathematics and two things in Mathlib.
A `Finset α` is a finite set held as *data* — you can sum over it, and `∑ i ∈ s, f i` means exactly that — while a `Set α` together with `Set.Finite` is a finite set held as a *proposition*; `Set.Finite.toFinset` turns the second into the first, and a `Finset` coerces back with `↑`.
When a whole type is finite that is `Fintype`, which is what lets `∑ i, f i` range over it with no set mentioned at all.

The other is decidability.
Writing something like "$`1` if $`i = j`, else $`0`" requires *deciding* whether $`i = j`, so such statements carry a `[DecidableEq ι]` assumption, and constructions over a finite index often want `[Fintype ι]` alongside.
This is bookkeeping, not mathematics: `Classical.decEq` decides any equality at the cost of being noncomputable, and `open scoped Classical` puts it to work when a definition only needs to exist rather than run.
That is also why so many objects here are marked `noncomputable` — see the quotient groups in the chapter on quotients.
