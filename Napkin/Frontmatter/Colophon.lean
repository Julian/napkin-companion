...
*Running it yourself.*
Every code block in the book is elaborated during the build, against the Mathlib and Lean versions pinned in the [companion's repository](https://github.com/Julian/napkin-companion); to attempt an exercise, copy it into a file whose imports match the ones at the top of that chapter's source.
