...
:::aside
This is Mathlib's {name}`PrimeSpectrum`: `PrimeSpectrum A` is the type whose terms are the prime ideals of $`A` (each bundled with a proof of primality).
...
:::aside
The vanishing locus is {name}`PrimeSpectrum.zeroLocus` (of a set or an ideal), and Mathlib puts exactly this topology on `PrimeSpectrum A`, generated by declaring the zero loci closed; the dual operation sending a set of points back to the ideal of everything vanishing on it is {name}`PrimeSpectrum.vanishingIdeal`.
:::
...
:::aside
The mandatory exercise is exactly {name}`PrimeSpectrum.isClosed_singleton_iff_isMaximal`.
...
:::aside
The correspondence is {name}`PrimeSpectrum.isIrreducible_iff_vanishingIdeal_isPrime`: a closed set is irreducible exactly when its vanishing ideal is prime, and that prime is the (unique) generic point.
:::
...
:::aside
This is {name}`ringKrullDim`, valued in $`\mathbb{Z} \cup \{\pm\infty\}` so that the zero ring lands at $`-\infty`; the topological version — chains of irreducible closed subspaces — is {name}`topologicalKrullDim`, and {name}`PrimeSpectrum.topologicalKrullDim_eq_ringKrullDim` says the two agree on a spectrum.
:::
...
:::aside
Mathlib packages this as {name}`PrimeSpectrum.vanishingIdeal_zeroLocus_eq_radical`: the ideal of functions vanishing on $`\mathbb{V}(I)` is exactly $`\sqrt I`, which unwinds to "$`\sqrt I` is the intersection of the primes containing $`I`".
The previous proposition is then {name}`PrimeSpectrum.zeroLocus_radical`.
:::
