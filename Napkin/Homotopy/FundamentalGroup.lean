...
The idea is that one loop of the eight is $`a`, and the other loop is $`b`, so we expect $`\pi_1` to be generated by these loops $`a` and $`b` (and their inverses $`\overline a` and $`\overline b`).
