...
```lean
example (Ω : Type*) (F : Set (Set Ω)) : MeasurableSpace Ω :=
  MeasurableSpace.generateFrom F
```
...
```lean
example (Ω : Type*) (s : Set Ω) : @MeasurableSet Ω ⊤ s :=
  MeasurableSpace.measurableSet_top

example (Ω : Type*) [TopologicalSpace Ω] : MeasurableSpace Ω := borel Ω
```
...
`Continuous.measurable` is exactly the "continuous implies Borel measurable" proposition in Mathlib (with `[BorelSpace Y]` and the standing convention that `X` carries its own Borel structure).

```lean
example {X Y : Type*} [TopologicalSpace X] [MeasurableSpace X]
    [OpensMeasurableSpace X] [TopologicalSpace Y] [MeasurableSpace Y]
    [BorelSpace Y] {f : X → Y} (hf : Continuous f) : Measurable f :=
  hf.measurable
```
