...
$$`\begin{array}{llll} \hline \text{Name} & \text{Domain } G & \text{Dual } \widehat{G} & \text{Characters} \\ \hline \text{Binary Fourier analysis} & \{\pm 1\}^n & S \subseteq \{1, \dots, n\} & \prod_{s \in S} x_s \\ \text{Fourier transform on finite groups} & Z & \xi \in \widehat{Z} \cong Z & e(\xi \cdot x) \\ \text{Discrete Fourier transform} & \mathbb{Z}/n\mathbb{Z} & \xi \in \mathbb{Z}/n\mathbb{Z} & e(\xi x / n) \\ \text{Fourier series} & \mathbb{T} \cong [-\pi, \pi] & n \in \mathbb{Z} & \exp(inx) \\ \hline \text{Continuous Fourier transform} & \mathbb{R} & \xi \in \mathbb{R} & e(\xi x) \\ \text{Discrete time Fourier transform} & \mathbb{Z} & \xi \in \mathbb{T} \cong [-\pi, \pi] & \exp(i \xi n) \\ \end{array}`
