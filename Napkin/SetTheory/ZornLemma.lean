...
These form a partially ordered set by inclusion, and what we did was quite literally climb up a chain $$`S_0 \subset S_1 \subset S_2 \subset \dots.`
...
Every chain $`S_0 \subset S_1 \subset \dots` has an upper bound $`\bigcup S_\alpha` (which you have to check is actually an element of the poset).
