...
:::aside
For functions on subsets of $`\mathbb{C}`, this signed order is `meromorphicOrderAt f p`:
...
The codomain `WithTop ℤ` hides a convention worth pausing on: the order of the zero function is defined to be $`\top` (think $`+\infty`: it vanishes to arbitrarily high order), which keeps identities like $`\operatorname{ord}_p(fg) = \operatorname{ord}_p(f) + \operatorname{ord}_p(g)` (that's `meromorphicOrderAt_mul`) true without a nonvanishing hypothesis.
:::
...
:::aside
This is one of the pieces of this chapter that already exists in Mathlib, as `Function.locallyFinsuppWithin U ℤ`: a $`\mathbb{Z}`-valued function bundled with proofs that its support lies in $`U` and is locally finite there.
("Locally finite" and "discrete" agree here — that equivalence is `supportDiscreteWithin_iff_locallyFiniteWithin`.)
Divisors so packaged add and subtract pointwise (they form a lattice-ordered commutative group), which is the next few sections' arithmetic.
:::
...
:::aside
$`\operatorname{Div}(f)` is `MeromorphicOn.divisor f U`, with the two Mathlib pieces above composing exactly as in the definition — the divisor's value at $`z` is `meromorphicOrderAt f z`, squashed back to `ℤ`:
...
The caveat remains the domain: `𝕜 → E` means divisors of meromorphic functions on (subsets of) $`\mathbb{C}` — the chapter's divisors on an arbitrary Riemann surface, and in particular anything mentioning $`\infty : \mathbb{C}_\infty`, still have no Mathlib counterpart.
:::
...
The Riemann-Roch theorem is not in Mathlib, in either its Riemann-surface or algebraic-curve incarnation, and neither are $`L(D)` spaces or canonical divisors; of this chapter's cast, only the divisor of a meromorphic function (`MeromorphicOn.divisor`, on subsets of $`\mathbb{C}`) has been formalized.
:::
