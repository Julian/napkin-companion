...
The claim we just proved is `minpoly.dvd`, stated over any base field:
...
(`Polynomial.aeval x p` is "evaluate `p` at `x`", with the coefficients mapped into the algebra where `x` lives — the general form of plugging a complex number into a rational polynomial.)
...
Both directions of that question have names too: `minpoly.irreducible` and `minpoly.eq_of_irreducible_of_monic`.
...
This proposition is the statement that $`\mathbb{Z}` is _integrally closed_ (in its fraction field $`\mathbb{Q}`) — a notion we will meet officially in two chapters — and it holds as the instance `IsIntegrallyClosed ℤ`, an instance available for any unique factorization domain:
...
(For the record, the closure facts are `IsIntegral.add` and `IsIntegral.mul`, and both integral closures come packaged as subalgebras — `integralClosure ℤ ℂ` — so the ring structure is part of the packaging.)
...
whose two fields are `CharZero K` — the honest way to say "contains $`\mathbb{Q}`" when the carrier is an abstract type rather than a subfield of $`\mathbb{C}` — and `FiniteDimensional ℚ K`.
The degree is then `Module.finrank ℚ K`.
...
The theorem is `Field.exists_primitive_element`, stated for any finite separable field extension `E` of `F` (both hypotheses automatic for number fields, where separability comes free with characteristic zero): there exists `α : E` with `F⟮α⟯ = ⊤`, i.e. adjoining $`\alpha` already gives everything.
