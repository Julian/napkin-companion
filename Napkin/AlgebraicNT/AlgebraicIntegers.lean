...
Both directions of the "irreducible $`\iff` minimal" question have names too: `minpoly.irreducible` and `minpoly.eq_of_irreducible_of_monic`; the first says the minimal polynomial of an integral element is irreducible.
...
example (α : ℂ) (p : Polynomial ℚ) (hp : Polynomial.aeval α p = 0)
  ...
  -- minpoly divides any vanishing polynomial, and dividing a nonzero
  -- polynomial cannot raise its degree.
  Polynomial.natDegree_le_of_dvd (minpoly.dvd ℚ α hp) hp0
```
:::
...
The sum of two algebraic integers is again an algebraic integer:
...
example (x y : ℂ) (hx : IsIntegral ℤ x) (hy : IsIntegral ℤ y) :
    IsIntegral ℤ (x * y + x) :=
  -- Closed under × then +: x*y is integral, and adding x keeps it so.
  (hx.mul hy).add hx
```
:::
...
Every number field is thus generated by a single primitive element:
...
The force of "$`F⟮α⟯ = ⊤`" is that *every* element of $`K` is reachable from $`\alpha`.
...
example (K : Type*) [Field K] [NumberField K] :
    ∃ α : K, ∀ x : K, x ∈ IntermediateField.adjoin ℚ {α} := by
  -- The primitive element generates everything, so ⊤ swallows every x.
