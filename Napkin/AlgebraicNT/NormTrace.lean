...
Note that in this case the conjugates of $`\alpha` do not lie in the field $`K`!
:::
...
The group generated by $`\{ 1, \alpha, \alpha^2 \}` has index 2 in $`\mathcal{O}_K` — that is, $`|\mathcal{O}_K/\langle 1, \alpha, \alpha^2\rangle|=2`, and we miss $`\beta`.

If we try to pick $`\{ 1, \beta, \beta^2 \}` as a basis instead, again we get $`|\mathcal{O}_K/\langle 1, \beta, \beta^2\rangle|=2`, and we miss $`\alpha`.
If you explicitly computing it out, you can get $`\beta^2 = \frac{3\alpha^2 + 7\alpha}{2} + 6 = 3 \beta+2 \alpha+6`.
